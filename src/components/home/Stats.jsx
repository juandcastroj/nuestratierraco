...
export default function Stats() {
  return (
    <>
      <div className="bg-[#004e64]  opacity-95 py-20 sm:py-24">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl lg:max-w-7xl">

            <div className="text-center text-[#9fffcb] sm:max-w-4xl mx-auto animate-fade-right animate-duration-[2500ms] animate-delay-[500ms]">
            ...
            <dl className="mt-10 grid grid-cols-1 gap-0.5 overflow-hidden rounded-2xl text-center sm:grid-cols-2 lg:grid-cols-4 animate-fade-left animate-duration-[2500ms] animate-delay-[500ms]">
              {stats.map((stat) => (
                <div key={stat.id} className="flex flex-col bg-white/10 p-8">
                  <dt className="text-base font-bold text-[#9fffcb] leading-normal mt-3">{stat.name}</dt>
                  <dd className="order-first text-4xl font-extrabold text-white">{stat.value}</dd>
                </div>
              ))}
            </dl>
